...
t_int *mysofa_tilde_perform(t_int *w) {
   ...
   // mysofa_getfilter_float(x->sofa,x->x,x->y,x->z,x->leftIR,x->rightIR,&x->leftDelay,&x->rightDelay);
    ...
    for(i =x->filter_length;i<(x->fftsize-x->filter_length);i++){
    ...
    /*
    for(i=x->convsize;i<x->fftsize-x->convsize;i++){
    ...
    if(x->flag == 0){
        ...
        for(i=0; i<x->filter_length; i++){
            x->buffer_right[i] = x->in_right_final[i+n][0];
            x->buffer_left[i] = x->in_left_final[i+n][0];
